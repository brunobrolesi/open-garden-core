// Code generated by mockery v2.26.1. DO NOT EDIT.

package mocks_middleware

import (
	middleware "github.com/brunobrolesi/open-garden-core/internal/api/middleware"
	mock "github.com/stretchr/testify/mock"
)

// TokenService is an autogenerated mock type for the TokenService type
type TokenService struct {
	mock.Mock
}

// ValidateToken provides a mock function with given fields: token
func (_m *TokenService) ValidateToken(token middleware.Token) (int, error) {
	ret := _m.Called(token)

	var r0 int
	var r1 error
	if rf, ok := ret.Get(0).(func(middleware.Token) (int, error)); ok {
		return rf(token)
	}
	if rf, ok := ret.Get(0).(func(middleware.Token) int); ok {
		r0 = rf(token)
	} else {
		r0 = ret.Get(0).(int)
	}

	if rf, ok := ret.Get(1).(func(middleware.Token) error); ok {
		r1 = rf(token)
	} else {
		r1 = ret.Error(1)
	}

	return r0, r1
}

type mockConstructorTestingTNewTokenService interface {
	mock.TestingT
	Cleanup(func())
}

// NewTokenService creates a new instance of TokenService. It also registers a testing interface on the mock and a cleanup function to assert the mocks expectations.
func NewTokenService(t mockConstructorTestingTNewTokenService) *TokenService {
	mock := &TokenService{}
	mock.Mock.Test(t)

	t.Cleanup(func() { mock.AssertExpectations(t) })

	return mock
}
